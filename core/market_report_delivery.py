...
def notify_market_report(user_id,report,run_key,send_fn=send_webhook):
    ...
    prior=load_desk_output(uid,output_type,run_key)
    if prior and (prior.get('payload') or {}).get('delivered'):
    ...
    if provider=='DISCORD':
        for index,embed in enumerate(build_market_report_embeds(report),1):
            results.append(bool(send_fn(f'{report.get("title")} · {index}',url=target,discord_embed=embed)))
    else:
        for chunk in _text_chunks(render_market_report_markdown(report)):
            results.append(bool(send_fn(chunk,url=target)))
    ...
    payload={'status':'DELIVERED' if delivered else 'FAILED','delivered':delivered,'attempted':True,
             'parts':len(results),'parts_delivered':sum(results),'report_version':report.get('schema_version')}
    save_desk_output(uid,output_type,payload,run_key=run_key); return payload
