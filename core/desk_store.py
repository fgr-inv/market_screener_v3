...
def _run_path(uid,typ,key):
    digest=hashlib.sha256(str(key).encode('utf-8')).hexdigest()[:20]
    return DIR/f'{_safe(uid)}_{_safe(typ)}_{digest}.json'
...
def load_desk_output(user_id, output_type, run_key):
    """Load one exact idempotent run rather than only the latest output."""
    uid=str(user_id or 'local-user'); typ=str(output_type); key=str(run_key)
    if cloud_available():
        df=query_sql('''SELECT created_at,payload_json FROM user_agent_outputs
                        WHERE user_id=:uid AND output_type=:typ AND run_key=:key LIMIT 1''',
                     {'uid':uid,'typ':typ,'key':key})
        if not df.empty:
            try: return {'user_id':uid,'output_type':typ,'run_key':key,'created_at':str(df.iloc[0]['created_at']),'payload':json.loads(df.iloc[0]['payload_json'])}
            except Exception: pass
    p=_run_path(uid,typ,key)
    if not p.exists(): return None
    try: return json.loads(p.read_text(encoding='utf-8'))
    except Exception: return None

def desk_output_exists(user_id, output_type, run_key):
    return load_desk_output(user_id,output_type,run_key) is not None
