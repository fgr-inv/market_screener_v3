...
st.dataframe(pd.DataFrame([[k,v] for k,v in cyc.items()],columns=['Decision dimension','Reading']),use_container_width=True,hide_index=True)
...
if score_rows: st.dataframe(pd.DataFrame(score_rows,columns=['Component','Score']),use_container_width=True,hide_index=True)
st.dataframe(pd.DataFrame([[k,v] for k,v in pro.items() if k not in {'Missing_Professional_Data','Framework'} and not str(k).endswith('_Score')],columns=['Metric','Value']),use_container_width=True,hide_index=True)
...
if choice in pm and pm[choice] is not None: st.plotly_chart(technical_chart(enrich_indicators(pm[choice]),choice),use_container_width=True)
