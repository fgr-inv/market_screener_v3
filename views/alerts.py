...
priority_news_scan=load_latest_desk_output(uid,'news_catalyst_priority_scan') or {}
daily_delivery=load_latest_desk_output(uid,'cio_daily_delivery') or {}
health_state=load_desk_output(uid,'automation_health_state','current') or {}
...
merged_news=merge_news_scan_records([priority_news_scan,news_scan]); news_events=merged_news['actionable_events']
material_news=merged_news['material_events']
...
st.caption(
    f"Portfolio news (30 min): {priority_news_scan.get('created_at','N/D')} · "
    f"Full news (60 min): {news_scan.get('created_at','N/D')}"
)
...
health_payload=health_state.get('payload') or {}; health_status=health_payload.get('status','PENDING')
c4.metric('Automation health',health_status)
if health_status=='DEGRADED':
    labels=', '.join(row.get('label',row.get('process','N/D')) for row in health_payload.get('issues') or [])
    st.error('Automatización atrasada: '+(labels or 'revisar GitHub Actions'))
elif health_status=='HEALTHY':
    st.caption(f"Automation watchdog: HEALTHY · {health_state.get('created_at','N/D')}")
...
section_note('The live controls on this page are manual. Saved Alerts and the portfolio/watchlist monitor run every 15 minutes during the US market session; the broad hunt remains daily even while the web is closed.')
